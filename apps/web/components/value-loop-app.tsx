...
function Queue({ openAccount, directory }: { openAccount: (accountId: string) => void; directory?: boolean }) {
  ...
  return <>
    ...
    {!directory && <section className="coverage-board"><SectionTitle eyebrow="Eight-path coverage" title="Churn is not one problem" detail="Each pathway has its own evidence, safe response, and seeded Customer 360." /><div>{churnProfiles.map((profile, index) => { const account = getAccount(profile.accountId); return <button key={profile.churnType} onClick={() => openAccount(account.id)}><span>{String(index + 1).padStart(2, "0")}</span><strong>{profile.churnType}</strong><small>{account.name}</small><em>{profile.action.recommended}</em><ArrowRight /></button>; })}</div></section>}
    <article className="card queue-card"><div className="queue-tools"><div className="tabs">{(directory ? ["All", "Enterprise", "Growth", "Team", "Healthy"] : churnTabs).map(x => <button className={tab === x ? "active" : ""} onClick={() => setTab(x)} key={x}>{x}{x === "Urgent" && <b>5</b>}</button>)}</div><div className="tool-actions"><label className="search"><Search /><input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search accounts or churn types" /></label><button className="secondary"><Filter />Filters</button></div></div>{!directory && <div className="filter-row"><span>Pathway: {tab}</span><span>Renewal: 90 days</span><span>Freshness: Current</span><button onClick={() => setTab("All")}>Clear all</button></div>}<AccountTable rows={filtered} onSelect={(account) => openAccount(account.id)} /><footer className="table-footer"><span>Showing {filtered.length} of {accounts.length} seeded accounts</span><div><button disabled><ArrowLeft />Previous</button><button>Next<ArrowRight /></button></div></footer></article>
  </>;
}
