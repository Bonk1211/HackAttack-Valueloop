...
The approved `design.md` is implemented as a local-fixture-only Next.js frontend in `apps/web/`. It includes real routes for the overview, risk queue, accounts, Customer 360, approvals, outcomes, and audit screens, plus responsive navigation, mock interactions, accessible focus states, and a custom 404. Eight deterministic profiles cover Value, Experience, Product-fit, Price, Involuntary, Competitive, Lifecycle, and Silent churn from evidence through governed action, outcome, and audit; account routes load the selected fixture. Customer 360 now exposes the bounded agent run as visible checkpoints, structured confidence and evidence references, agent and policy versions, eligible and rejected actions, and local approve, modify, and reject transitions. The Risk Queue defaults to an interactive account → pathway → leading-issue map with a table alternative. A judge-friendly `/guided-demo` route now explains the complete Detect → Explain → Decide → Approve → Act → Measure loop in non-technical language, and `/playbooks` demonstrates safe no-code customization of scenario templates, confidence thresholds, frequency caps, approval ownership, customer-choice protections, and outcome fields. Every screen includes a contextual Page tutorial that scrolls to, spotlights, and explains the actual visible interface elements with keyboard-accessible step controls. The stakeholder-reviewed visual language uses regular-weight warm-neutral typography, bracketed monograms, a movie-ticket-inspired selected-account risk pass, and a light evidence-file treatment for cause hypotheses. Motion is limited to spring-based shell and section entrances, transform/opacity state changes, and tactile control feedback; reduced-motion users receive immediate state changes. The production frontend is deployed at `https://web-livid-beta-ilnxxzodh3.vercel.app`; this URL may not include later local mockup revisions until redeployed. Backend integration, persistence, authentication, and production intelligence are deliberately excluded.
